...
public class S3Upload {
    ...
    private static String AWS_ACCESS_KEY = "AKIAOAYZXO3JUCFTYA6A";
    private static String AWS_SECRET_KEY = "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";
    private static String bucket_name = "smc-unilever";
...
//            XferMgrProgress.waitForCompletion(xfer);
    ...
    public static String uploadToS3(File tempFile, String remoteFileName) throws Exception {
...
//            throw new PgCsDocException("error occurs during upload to s3!");
            System.out.println();
     ...
     */
    @SuppressWarnings({ "deprecation", "unused" })
    public static String uploadToS3(InputStream inputStream, String key) throws Exception {
...
//			}
             ...
             * 设置过期时间
             */
            urlRequest.setExpiration(DateUtils.addDays(new Date(),7));
//            urlRequest.setExpiration(DateUtil.getDateByPeriod(new Date(), 7));
     ...
     * */
    @SuppressWarnings({ "deprecation", "unused" })
    public static String uploadToS3(File tempFile) throws Exception {
        //首先创建一个s3的客户端操作对象（需要amazon提供的密钥）
        s3 = new AmazonS3Client(new BasicAWSCredentials(AWS_ACCESS_KEY, AWS_SECRET_KEY));
        Region usWest2 = Region.getRegion(Regions.CN_NORTH_1);
        s3.setRegion(usWest2);
...
//			}
            ...
            Date expirationDate = null;
            try {
                expirationDate = DateUtils.parseDate("2020-12-31",new String[]{"yyyyMMdd"});
//                expirationDate = DateUtils.yyyyMMdd.parse("2020-12-31");
            } catch (Exception e) {
                e.printStackTrace();
            }
            //设置过期时间
//			urlRequest.setExpiration(expirationDate);
    ...
    @SuppressWarnings({ "deprecation", "unused" })
    public static String getFileUrlByKey(String key) throws Exception {
        //首先创建一个s3的客户端操作对象（需要amazon提供的密钥）
        s3 = new AmazonS3Client(new BasicAWSCredentials(AWS_ACCESS_KEY, AWS_SECRET_KEY));
        Region usWest2 = Region.getRegion(Regions.CN_NORTH_1);
        s3.setRegion(usWest2);
...
//		}
        //3.
        File file = new File("/Users/haoxueqiang/test/test1/hao.zip");
        try {
            uploadToS3(new FileInputStream(file),"hao222.zip");
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
        }
